...
/** @deprecated Use SessionZeroSectionGroup */
export type SessionZeroCategoryGroup = SessionZeroSectionGroup;
...
export function getSessionZeroQuestion(id: string): SessionZeroQuestion | undefined {
	return SESSION_ZERO_QUESTIONS.find((question) => question.id === id);
}
...
export function sortSessionZeroQuestionIds(questionIds: string[]): string[] {
	return SESSION_ZERO_QUESTION_IDS.filter((id) => questionIds.includes(id));
}
...
/** @deprecated Use groupActiveQuestionsBySection */
export function groupActiveQuestionsByCategory(activeQuestionIds: string[]): SessionZeroSectionGroup[] {
	return groupActiveQuestionsBySection(activeQuestionIds);
}

/** @deprecated Use groupExcludedQuestionsBySection */
export function groupExcludedQuestionsByCategory(
	activeQuestionIds: string[]
): SessionZeroSectionGroup[] {
	return groupExcludedQuestionsBySection(activeQuestionIds);
}
...
function normalizeAnswers(raw: Record<string, unknown>): Record<string, string> {
	...
	for (const [key, value] of Object.entries(raw)) {
		if (SESSION_ZERO_QUESTION_IDS.includes(key) && typeof value === 'string') {
			answers[key] = value;
		}
	}
