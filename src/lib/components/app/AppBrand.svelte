<script lang="ts">
	...
	const brandHref = $derived.by(() => {
		...
		if (partId && adventureId) {
			return resolve('/campaigns/[campaignId]/adventures/[adventureId]/parts/[partId]', {
				campaignId,
				adventureId,
				partId
			});
		...
		if (adventureId) {
			return resolve('/campaigns/[campaignId]/adventures/[adventureId]', {
				campaignId,
				adventureId
			});
		}
...
<a href={brandHref} class="brand" class:brand--stacked={brand.kind !== 'app'}>
	{#if brand.kind === 'app'}
